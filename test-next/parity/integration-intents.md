...
| Exact production event contracts                     | Event assertions distributed through market/integration suites                                               | Several events are asserted, but nine production event identifiers disappeared from the replacement tree                                         | Missing assertions | Restore exact emitter/topic/data assertions with the smallest existing domain owner                                                                                                                              |
| Lens rejects non-V2 markets with the canonical error | Legacy Lens bubble test                                                                                      | Facade tests generic delegate reverts and malformed probes                                                                                       | Missing assertion  | Add an exact `NotV2Market` production-path assertion to `MarketLensFacadeTest`                                                                                                                                   |

## Missing exact event assertions

The replacement tree currently has no explicit assertion for these production events:
...
An event is not considered preserved merely because the state transition is covered. Event shape,
emitter, indexed fields, and payload are downstream protocol contracts for indexers and services.
