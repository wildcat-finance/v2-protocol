...
Twenty-nine are market and borrower-identity behavior. The replacement maps those entries into ten
composed properties and adds one adversarial property for malformed registry return data. It uses
production ArchController, borrower registry, sanctions Sentinel, OpenTerm hooks, and market code,
with artifact-backed deployment and one concrete suite.
...
The legacy fixed-seed oracle passes all 37 entries. The 11 replacement properties pass with the
same timestamp, seed, and 1,000 fuzz runs. Focused accurate coverage hits all 64 executable lines
