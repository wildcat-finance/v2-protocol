...
import { BorrowingBaseLib } from 'src/access/covenants/lib/BorrowingBaseLib.sol';
import { CrossMarketCapLib } from 'src/access/covenants/lib/CrossMarketCapLib.sol';
...
import { CommitmentScheduleHooks } from 'src/access/CommitmentScheduleHooks.sol';
import { DrawTimelockHooks } from 'src/access/DrawTimelockHooks.sol';
...
import { CREATE2_DEPLOYER, CROSS_MARKET_GATE_LIB, CLEAN_DOWN_LIB, COMMITMENT_SCHEDULE_LIB, DRAW_TIMELOCK_LIB, BORROWING_BASE_LIB, CROSS_MARKET_CAP_LIB, CROSS_MARKET_GATE_LIB_SALT, CLEAN_DOWN_LIB_SALT, COMMITMENT_SCHEDULE_LIB_SALT, DRAW_TIMELOCK_LIB_SALT, BORROWING_BASE_LIB_SALT, CROSS_MARKET_CAP_LIB_SALT } from 'src/access/covenants/lib/CovenantLibraries.sol';
...
contract DeployCovenantHooksTemplatesV26 is V25DeployScriptBase {
  string internal constant COVENANT_ARTIFACT =
    'src/access/CombinedCovenantHooks.sol:CombinedCovenantHooks';
  string internal constant CLEANDOWN_ARTIFACT =
    'src/access/CleanDownHooks.sol:CleanDownHooks';
  ...
  function _ensureCovenantLibraries(bool planMode) internal {
    ...
    _ensureLibrary(
      ...
      'BorrowingBaseLib',
      BORROWING_BASE_LIB,
      BORROWING_BASE_LIB_SALT,
      type(BorrowingBaseLib).creationCode
    );
    _ensureLibrary(
      planMode,
      'CrossMarketCapLib',
      CROSS_MARKET_CAP_LIB,
      CROSS_MARKET_CAP_LIB_SALT,
      type(CrossMarketCapLib).creationCode
    );
  }
  ...
  function run() external {
    ...
    if (!vm.envOr('SKIP_CLEANDOWN_TEMPLATE', false)) {
      _deployAndRegister(
        ...
        'CleanDownHooks',
        CLEANDOWN_ARTIFACT,
        type(CleanDownHooks).creationCode
      );
    }
  }
  ...
  /// @dev `CombinedCovenantHooks` -> `revolving-covenant-hooks`, for plan ids.
