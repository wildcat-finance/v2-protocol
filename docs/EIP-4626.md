...
- **Direct transfers** of the market token to the wrapper do **not** mint shares; those tokens become **stranded**.
...
`sweep(token, to)` allows the market’s borrower (captured as `marketOwner`) to rescue arbitrary ERC‑20 balances from the wrapper **except** the wrapped market token itself:

- Only callable by `marketOwner`.
- Reverts if `token == asset()` (market token can’t be swept).
- Reverts if `to` is sanctioned.
- Sweeps the entire balance and emits `TokensSwept(token, to, amount)`.
...
- `CannotSweepMarketAsset()` — attempted to sweep the wrapped market token.
...
- **Don’t transfer the market token directly to the wrapper.** You will not receive shares, and the wrapper can’t sweep the market token back out; those tokens become stranded (`test_totalAssetsTracksDirectTransfers` demonstrates this).
