...
Redeeming from the wrapper returns the **market token**, not the market’s underlying asset. To exit to the underlying asset you must follow the Wildcat market’s withdrawal flow (withdrawal request → batch expiry → execution/claim). See `docs/protocol/withdrawals.md` and `docs/Terminology.md` (“Withdrawal Request”, “Claim”, etc).
