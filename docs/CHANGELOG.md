...
- The half-up `MarketState.scaleAmount` was removed; rounding directions are documented in [Scale Factor](./protocol/scaling-and-rounding.md#rounding).
...
- Added borrower transfer events and v2.5 lens fields for current and pending principals, pending borrower, and identity-registry discovery. See [Borrower Identity and Transfers](./protocol/borrower-identity.md).
