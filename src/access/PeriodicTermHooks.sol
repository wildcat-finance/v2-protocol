...
contract PeriodicTermHooks is BaseAccessControls, MarketConstraintHooks {
    ...
    event PeriodicTermUpdated(
        address market, uint32 periodStart, uint32 periodDuration, uint32 withdrawalWindowDuration
    );
    ...
    constructor(address _deployer, bytes memory args) BaseAccessControls(_deployer) IHooks() {
        ...
        HooksConfig requiredFlags = EmptyHooksConfig.setFlag(Bit_Enabled_SetAnnualInterestAndReserveRatioBips)
            .setFlag(Bit_Enabled_QueueWithdrawal).setFlag(Bit_Enabled_CloseMarket);
    ...
    function _readUint128Cd(bytes calldata data, uint256 offset) internal pure returns (uint128 value) {
        uint256 _value;
        assembly {
            _value := calldataload(add(data.offset, offset))
        }
    ...
    function _onCreateMarket(
        ...
        _validatePeriodicTerm(periodDuration, withdrawalWindowDuration);
        ...
        if (marketHooksConfig.useOnQueueWithdrawal()) {
            marketHooksConfig = marketHooksConfig.setFlag(Bit_Enabled_Transfer).setFlag(Bit_Enabled_Deposit);
        }
    ...
    function _validatePeriodicTerm(uint32 periodDuration, uint32 withdrawalWindowDuration) internal pure {
    ...
    function onDeposit(address lender, uint256 scaledAmount, MarketState calldata state, bytes calldata hooksData)
        external
        override
    {
        ...
        (bool hasValidCredential, bool roleUpdated) = _tryValidateAccessInner(status, lender, hooksData);
    ...
    function onQueueWithdrawal(
        ...
        HookedMarket memory market = _hookedMarkets[msg.sender];
        if (!market.isHooked) revert NotHookedMarket();
        if (!state.isClosed && !_isWithdrawalWindowOpen(market, block.timestamp)) revert WithdrawOutsideWindow();
