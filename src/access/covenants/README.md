...
Composable conditions-precedent for Wildcat markets.

Every credit facility in traditional finance gates each drawing on
...
are. These contracts add six covenants that are computable from chain state
alone, plus the architecture to add more: a cross-market delinquency gate, a
clean-down requirement, a commitment-reduction schedule, a draw timelock, a
borrowing base over on-chain collateral, and an aggregate cross-market
exposure cap.
...
Five of the six are **drawstops**. Breach one and new draws are blocked, and
...
facility that draws on demand, it needs no third party to invoke it, and it
can't be waived by inattention.

The sixth works the other way round.

One scoping note: **covenant templates bind markets from either wildcat
factory**, standard and revolving alike. Covenants are facility furniture,
not revolver furniture. The drawn-amount prediction used on the borrow path
converges to the same value for both market kinds: natively reported drawn
on revolving markets, and debt not covered by assets still in the market on
standard ones. The draw timelock doesn't stop anything;
...
| `BorrowingBaseCovenant.sol` + `lib/BorrowingBaseLib.sol` | Borrowing base: collateral ledger, custody surface, library bodies |
| `CrossMarketExposureCapCovenant.sol` + `lib/CrossMarketCapLib.sol` | Aggregate exposure cap: mixin surface plus library bodies |
| `WatchedMarketsBase.sol` | Shared watch-list: storage and permissionless add/prune, inherited by the gate and the cap |
| `ModularCovenants.sol` | Runtime covenant composition: append-only per-market module lists, waiver gate, `STATICCALL` dispatch |
| `CovenantModuleRegistry.sol` | Governed append-only module registry with pinned codehashes and the on-chain waiver text |
| `lib/ICovenantModule.sol` | The module predicate interface: `checkOnBorrow` and `validateConfig` |
| `modules/MaxDrawnModule.sol` | Reference module: a hard ceiling on drawn amount |
...
| `../CombinedCovenantHooks.sol` | Deployable template: gate and clean-down |
| `../CleanDownHooks.sol` | Deployable template: clean-down only |
| `../CommitmentScheduleHooks.sol` | Deployable template: commitment schedule only |
| `../DrawTimelockHooks.sol` | Deployable template: draw timelock only |
...
| `../BorrowingBaseHooks.sol` | Deployable template: borrowing base only |
| `../ExposureCapHooks.sol` | Deployable template: aggregate exposure cap only |
| `../ModularHooks.sol` | Deployable template: runtime covenant composition through the module registry |
| `../../libraries/DrawnMath.sol` | Pure drawn-amount transitions |
...
The obvious alternative is one dispatcher hook holding a list of covenant
module addresses, which buys runtime composability at the price of a module
registry. The registry itself is not the problem: the archcontroller is
already a governed registry, and every template here passes through it. The
compellable thing is *mutability*, a list that can reach into a live market
and change what it enforces.

So both designs exist here. Covenants are **abstract mixins composed at
compile time** by default, and `ModularHooks` carries the
dispatcher with the mutability removed: an append-only module registry governed by the same
archcontroller owner who curates templates, borrower-only appends to live
markets that are irrevocable by construction, modules reached by `STATICCALL`
as stateless predicates whose parameters live in the dispatcher, codehashes
pinned at registration and re-checked on every call, and a waiver whose hash
the borrower acknowledges in the append transaction itself. Appends only ever tighten, so self-binding is credible precisely because it
cannot be undone, which is the amendment mechanic compile-time composition
can't offer. The section on it below carries the details.

Which leaves an obvious question, so here is the decision rule. **A covenant
is a module if and only if it is a stateless borrow-gate**: a pure predicate
of the proposed draw, the chain, and parameters the dispatcher holds. State,
custody, repayment observation or exit machinery make it a mixin, full stop.
Run the six shipped covenants through that rule and each lands where it does
for a nameable reason: the gate and the exposure cap iterate a live
watch-list, the clean-down watches repayments across time, the timelock is an
announcement machine with a write surface, and the borrowing base holds
actual tokens, which is a custody story that belongs in an audited template
and not in third-party code under a waiver. Only a drawn ceiling or a
schedule-shaped commitment test is predicate-shaped, and `MaxDrawnModule` is
the former. The statelessness of the module interface is not a limitation to
engineer around; it is the entire reason the registry, the waiver and the
append can be offered safely, and extending modules to hold state would not
converge the two systems, it would rebuild mixins as mutable external
contracts and lose the isolation that made runtime composition acceptable.
...
- Past what the borrower sets at market creation, nothing is configurable
  after deployment: no owner, no upgrade path. The one deliberate exception
  is the modular template, where the borrower may irrevocably *append*
  covenant modules to their own market. Nothing may ever be removed, there
  either.

`CleanDownHooks` exists partly so that claim stays tested instead of
...
| --- | --- | --- |
| CombinedCovenantHooks | ~21,400 | ~3,100 |
| CleanDownHooks | ~19,900 | ~4,600 |
| CommitmentScheduleHooks | ~20,100 | ~4,400 |
| DrawTimelockHooks | ~20,100 | ~4,400 |
| FixedTermScheduleHooks | ~20,600 | ~3,900 |
| PeriodicTimelockHooks | ~21,400 | ~3,100 |
| BorrowingBaseHooks | ~21,200 | ~3,400 |
| ExposureCapHooks | ~20,200 | ~4,400 |
| ModularHooks | ~21,300 | ~3,300 |

The saving is per covenant, which is what made the third and fourth cheap
and keeps a fifth and sixth affordable. Each body stays at its own address
and the template carries only the dispatch. One covenant is the exception:
the modular dispatcher in `ModularCovenants` is inlined rather than
librarified, roughly double a librarified mixin's surface, which is
affordable in a template it has to itself and is exactly why a template
combining it with two or more mixins does not fit under the ceiling. Give
it a library before building that template.
...
Six covenants ship today across nine templates: each covenant behind its
own open-term single-covenant template, the gate and clean-down together in a
combined one, two term-structured templates carrying the covenants that earn
their keep there, a fixed-term schedule and a periodic window-aware timelock,
and a modular template that composes registry-reviewed covenant modules at
runtime rather than compile time. What follows is the behaviour a market actually gets; the covenant
...
And their errors and events are declared locally rather than in
`ICovenantEvents`, which exists to share declarations between each covenant
and its library so that library-raised errors stay in the template ABI. Host
behaviour has no library, so it has nothing to share. (Adding declarations to the shared file is harmless to library addresses,
incidentally: this repo compiles with `bytecode_hash = 'none'`, so interface
edits that generate no code leave every library's bytecode, and with it its
CREATE2 address, exactly where it was. Recomputing any pinned address after
such an edit verifies this directly.)
...
Near-universal in revolving facilities. The drawn balance has to return to a
de minimis level for `duration` consecutive seconds at least once every
`interval` seconds, which evidences use as a revolver rather than disguised
term debt.

"Clean" means drawn at or below a derived threshold rather than exactly
zero, because exact zero is not a stable target on a live market: interest
and protocol fees accrue continuously, so a facility funded to the wei is a
wei short one block later, and a covenant that fails on fee drift rather
than credit behaviour measures the wrong thing. The threshold is the larger
of two quantities, and it is derived rather than configured, so it is not a
lever anyone can set generously: a floor of one tenth of a token in the
market asset's own decimals, and one window's expected carry, the interest
plus protocol fee that current supply accrues over `duration`. The floor
absorbs rounding on small markets; the carry term scales with exactly the
thing that causes the drift, and on any sizeable market it is the binding
one, since a single block of accrual on a large facility already exceeds a
tenth of a token. Both are economically de minimis against any real
facility, and unpaid interest at scale sits far above the threshold, so
repaying principal while leaving real interest owed does not count as clean.
`cleanDownThreshold(market)` reports the live value.

Enforcement is a drawstop. Once overdue, draws that would leave the market
drawn beyond the threshold revert, and drawing resumes as soon as a fresh
qualifying streak completes.
...
- **Reclaiming an over-repayment isn't credit.** A draw that leaves the
  drawn amount within the threshold neither trips the covenant nor breaks a
  running streak.

Partial repayments don't start a streak. Only reaching the threshold does.
...
the archcontroller and reports this instance's borrower, which reaches markets
...
Availability capped at posted collateral times an advance rate: draws that
would take the drawn amount above the borrowing base revert, and collateral
only leaves down to what keeps the base covering the drawn amount.

This is the **static-haircut variant**, and that scoping is the covenant's
honesty. Per-token advance rates are fixed at market creation and are the
lender-accepted haircuts, exactly as advance rates function in an asset-based
facility. Collateral is valued at face in the market's own asset terms; the
covenant never re-marks it, so price movement between the tokens and the
asset sits with lenders, priced by the rates they accepted at entry. Suitable
where the tokens track the asset closely. Anything needing a live price feed
is an attested covenant wearing an oracle, and belongs in that half of the
map.

Custody is an internal per-market ledger on the hooks instance. Deposits move
tokens in through `transferFrom` and credit the ledger, and only ledger
balances count toward the base, so tokens sent directly to the instance are
ignored rather than silently inflating one market's collateral on an instance
several markets share. Anyone may deposit, since a donation only raises the
base; withdrawal is borrower-only and checked against the base at the
market's live drawn level, with closed markets exempt, because closure
settles the facility and stranding collateral past it protects nobody. Up to
8 collateral tokens per market, rates validated nonzero and at most 100%,
duplicates rejected, all at creation.

## Aggregate exposure cap

The incurrence-test analogue: a ceiling on the borrower's total debt across
their watched wildcat markets, checked whenever a market's drawn amount would
increase. It shares `WatchedMarketsBase` with the delinquency gate, so a
template carrying both maintains one list, and every market deployed through
a cap template is watched automatically at creation.

The debt metric is uniform in meaning rather than in method: what the
borrower has out. Revolving markets report it natively as drawn, for
standard markets it is outstanding debt not covered by assets still in the
market, and neither committed-but-undrawn capacity nor deposits the borrower
never touched count as exposure.

One property has to be stated wherever this covenant is described: **the
aggregate is a floor on the borrower's true exposure, never a
ceiling.** Every unwatched market is simply invisible, nothing on-chain
enumerates a borrower's markets exhaustively, and the number counts wildcat
debt only. It limits concentration within the protocol; it does not limit
leverage. The saving grace is that anyone can tighten it permissionlessly by
watching another of the borrower's markets, and a dedicated test pins the
floor semantics so nobody mistakes them for a guarantee.

## Modular covenants

`ModularHooks` inverts the compile-time rule under three invariants,
and every one of them removes a way for the design to be leaned on.

**Append-only, at both levels.** `CovenantModuleRegistry` is governed by the
archcontroller owner, the same gate that curates hooks templates, and it has
no removal, no replacement and no disable switch: nothing a live market
relies on can be reached from it, and a registry cannot be compelled to do
what it has no code to do. The per-market list is append-only too, and
borrower-only, which is what makes appending *mean* something: a borrower who
could loosen covenants mid-life would be offering lenders nothing. Appends
are irrevocable, they only ever tighten, and that is the amendment mechanic,
the thing facilities do constantly in traditional finance and compile-time
mixins cannot do at all short of closing the market and redeploying.

**`STATICCALL` isolation.** Modules are stateless predicates implementing
`ICovenantModule`: observe a proposed draw, revert to block it. Their
parameters live in the dispatcher and arrive as `config` on every call, so a
module holds no state, reaches no storage, and touches no roles. The worst a
hostile module can do is block draws on the market whose borrower chose to
append it, and because repayment and closure never consult modules, a
poisoned market unwinds normally: repay, close, done. There's a test that
does exactly that.

**Codehash pinned end to end.** The registry pins each module's codehash at
registration, the append copies the pin, and every dispatch re-checks it,
failing closed. A covenant that cannot be evaluated must not silently pass.

The waiver is on-chain and structural rather than ceremonial: the registry
carries the text, and `appendCovenantModule` requires its hash as an
argument, so the acknowledgement that covenant modules are third-party code
appended at the borrower's sole risk sits in the same transaction as the act
that takes the risk.

The registry pointer is per-market configuration, fixed at creation like
every other covenant parameter. Lenders should verify
`covenantModuleRegistry(market)` against the canonical deployment before
treating a market's appended covenants as meaningful, and can read the full
appended list, configs and pinned codehashes included, from
`getAppendedModules`.

`MaxDrawnModule` ships as the reference predicate, a hard ceiling on drawn
amount in one config word, mostly to keep the interface honest and the tests
concrete.
...
`CleanDownHooks` uses `0x00` to `0x60` only.
`CommitmentScheduleHooks` takes standard ABI encoding rather than fixed words,
...
`BorrowingBaseHooks` takes standard ABI encoding:
`abi.encode(uint128 minimumDeposit, bool transfersDisabled, address[] tokens,
uint16[] advanceRatesBips)`. Empty arrays disable the covenant.

`ExposureCapHooks` takes one covenant word: `0x40`, `uint128`, the
aggregate debt ceiling, with zero disabling the covenant.

`ModularHooks` takes standard ABI encoding: `abi.encode(uint128
minimumDeposit, bool transfersDisabled, address registry)`. Covenant modules
arrive after creation through `appendCovenantModule`, never through
`hooksData`.

`FixedTermScheduleHooks` extends the schedule template's ABI encoding with
the term as a third head:
...
fixed term in the past or beyond 365 days, a periodic schedule outside its
bounds, and a collateral configuration with duplicate tokens, a zero or
above-100% advance rate, or mismatched array lengths.

Covenant templates bind through either wildcat hooks factory.
`CovenantHooksCore`'s constructor checks the deploying factory's name and
accepts both; anything else is refused, so an instance can only exist behind
a genuine wildcat factory. The drawn metric is resilient to market kind:
read natively where the market reports `drawnAmount()`, and derived as debt
not covered by assets still in the market otherwise.
...
actually ask for. A feature-complete superset isn't on the menu from mixins alone, which is
why six covenants ship across eight compile-time templates rather than one,
and it's half of why the modular template exists: modules are separate
contracts, so the dispatcher escapes the ceiling entirely and the covenant
count can grow without template combinatorics growing with it.

---

# The covenant map

An index of what a facility can enforce, split by whether it needs a trust
root. The scores order the implemented set and the candidate list alike, and
the attested half doubles as a target list for a sealed entity credentials
...
Scoring covenants by traditional-finance universality gets the order wrong,
because TradFi lenders are always locked in and Wildcat lenders mostly
aren't. Four structural
...
**The base protocol already carries the asset-based-lending cushion.** In
asset-based lending (ABL), the availability floor and the consequences of
breaching it do most of the protective work. Wildcat has that natively: the
reserve ratio is a hard liquidity floor enforced on the borrow path, and
breaching it through withdrawals makes the market delinquent and springs
penalty APR after grace. That is excess-availability protection with a
springing consequence, on every market. Covenants that reinvent it are
double-counting.
...
`CommitmentScheduleCovenant`, shipped in `CommitmentScheduleHooks`. A
...
`DrawTimelockCovenant`, shipped in `DrawTimelockHooks`. Announce a draw
...
- **Open-term** (`DrawTimelockHooks`): constant delay of at least the
...
### Borrowing base over on-chain collateral (79/100)

`BorrowingBaseCovenant`, shipped in `BorrowingBaseHooks` as the
static-haircut variant. Availability capped at posted collateral times a
per-token advance rate fixed at creation.
...
about collateral coverage, so nothing here is double-counted. The cost score reflects what even the static-haircut core is: a custody
surface, a ledger, and a withdrawal gate, where most of its siblings are a
check.

Conditional in a way the others aren't, and the shipped version respects the
condition precisely. It's endogenous **only** where the collateral and its
valuation are both on-chain, so what ships values collateral at face with
static haircuts and no oracle: the honest endogenous core. Off-chain
collateral, and equally an oracle-marked on-chain version, are attested
covenants and live in the section below. If tokenised collateral tracking
live prices turns up in a market, the oracle-fed variant becomes the priority
and it arrives with a trust root attached.

### Cross-market aggregate exposure cap (65/100)

`CrossMarketExposureCapCovenant`, shipped in `ExposureCapHooks`. A
ceiling on the borrower's total debt across watched markets, not just this
one.

*Lender value 17 · legibility 16 · cheapness 15 · fidelity 6 · composition 11*

The incurrence-test analogue (a cap checked only when new debt is taken on,
never in between), and cheap because it reuses the watch-list the delinquency
gate already maintains. But it inherits that list's incompleteness
...
borrower's markets exhaustively. A cap that undercounts is a cap that lies, and it caps wildcat debt rather
than debt in any case, which is why the floor semantics are stated wherever
this covenant is described and a dedicated test pins them. Describe it
as limiting concentration within the protocol, never as limiting leverage,
and treat the number as a floor on exposure rather than a ceiling. Its one
structural courtesy: the list is permissionless, so any lender who knows of
another market can tighten the cap by watching it.

### 1. On-chain control change (64/100)
...
borrowers, since a plain externally owned account handing over its key is
invisible, and its value
...
### 3. Destination-constrained draws (59/100)
...
### 4. Excess-availability springing regime (58/100)
...
### 5. On-chain negative pledge (55/100)
...
|   | Borrowing base, on-chain collateral | 79 | Implemented, static-haircut variant; oracle-fed is attested |
|   | Cross-market aggregate exposure cap | 65 | Implemented; a floor on exposure, never a ceiling |
| 1 | On-chain control change | 64 | Smart-account borrowers; fixed-term host now available |
| 2 | Sweep-before-draw | 62 | Pair with destination constraints |
| 3 | Destination-constrained draws | 59 | Blocked on borrow path |
| 4 | Excess-availability springing | 58 | Mostly native already; one pre-breach tier survives |
| 5 | On-chain negative pledge | 55 | Dominated by borrowing base |
...
**The list is now demand-bound, not infrastructure-bound.** Six of the
original eleven are built, the hosts exist as mixins, and the shared
watch-list serves two covenants from one surface. A term-structured template
is now a composition, not a project. Of what remains, candidate 1 wants a
smart-account borrower to be worth much, 2 and 3 want the borrow-path
destination question settled, and 4 mostly exists natively. Nothing left on
the endogenous list is blocked on architecture; each is waiting for a
borrower or a lender to ask.

**Each covenant says exactly what it does.** The timelock treats the exit
right as the protection to amplify and is window-aware where the host has
windows; the borrowing base is exactly as wide as its endogenous condition
allows and not a token wider; the cap wears its floor semantics on the
outside. The map's job was never to flatter the covenants. It is to make
sure the implemented set says what it does and the candidate list says what
it would cost.
...
| Entity standing | Registered, in good standing, no insolvency filing | Registry, know-your-business (KYB) provider |
...
The issuer column matters. These aren't one role. Financial ratios belong to
an auditor, collateral valuation to an administrator, and entity standing,
...
- **`DrawnMath` isn't called by the market.** The hooks predict the
