...
interface ICovenantEvents {
  ...
  event AggregateExposureCapSet(address indexed market, uint256 cap);
  error AggregateExposureExceeded(uint256 aggregate, uint256 cap);

  event CollateralConfigSet(address indexed market, address[] tokens, uint16[] advanceRatesBips);
  event CollateralDeposited(address indexed market, address indexed token, address from, uint256 amount);
  event CollateralWithdrawn(address indexed market, address indexed token, address to, uint256 amount);
  error InvalidCollateralConfiguration();
  error WithdrawExceedsCollateral();
  error BorrowingBaseExceeded(uint256 drawnAfter, uint256 borrowingBase);
  error WithdrawalBreachesBase(uint256 borrowingBase, uint256 drawn);

  event CovenantModuleAppended(address indexed market, address indexed module, bytes config);
  error InvalidModuleRegistry();
  error NotOwnMarket();
  error WaiverNotAcknowledged();
  error ModuleNotRegistered();
  error ModuleCodehashMismatch();
  error TooManyModules();
  error ModuleAlreadyAppended();
  error CovenantModuleBlockedDraw(address module);
}
