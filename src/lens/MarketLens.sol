...
contract MarketLens {
  ...
  function _getAggregatedHooksInstancesForBorrowerInternal(
    address borrower,
    address[] memory factories
  ) internal view returns (HooksInstanceData[] memory arr) {
    uint256 numFactories = factories.length;
    if (numFactories == 0) {
  ...
  function _getAggregatedAllHooksTemplatesForBorrowerInternal(
    address borrower,
    address[] memory factories
  ) internal view returns (HooksTemplateData[] memory data) {
    uint256 numFactories = factories.length;
    if (numFactories == 0) {
  ...
  function getAggregatedHooksTemplatesForBorrowerWithFactory(
    address borrower
  ) public view returns (FactoryScopedHooksTemplateData[] memory data) {
    address[] memory factories = _getActiveHooksFactories();
    uint256 numFactories = factories.length;
    if (numFactories == 0) {
