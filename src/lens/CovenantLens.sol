...
import { CombinedCovenantHooks } from '../access/CombinedCovenantHooks.sol';
...
contract CovenantLens {
  function kindForVersion(string memory version) public pure returns (CovenantHooksKind) {
    ...
    if (h == keccak256('CombinedCovenantHooks')) return CovenantHooksKind.RevolvingCovenant;
    if (h == keccak256('CleanDownHooks')) return CovenantHooksKind.RevolvingCleanDown;
   ...
   *      `CombinedCovenantHooks.onBorrow`, including the exemption for draws
  ...
  function previewDraw(
    ...
    uint256 drawnAfter = DrawnMath.drawnAfterBorrow(
      data.drawnAmount,
      market.totalAssets(),
      state,
      amount
  ...
  function _fillCleanDown(MarketCovenantData memory data, address market) internal view {
    CombinedCovenantHooks hooks = CombinedCovenantHooks(data.hooksInstance);
