...
contract ERC5484RoleProviderTest is BaseMarketTest {
  ...
  function test_deposit_allows_erc5484_multiple_burn_auth() external {
    ...
    vm.startPrank(parameters.borrower);
    hooks.addRoleProvider(address(openProvider), 0);
    vm.stopPrank();
  ...
  function test_validateCredential_rejects_malformed_token_responses() external {
    ...
    vm.mockCall(
      address(nft),
      abi.encodeWithSelector(IERC5484BurnAuth.burnAuth.selector, tokenId),
      new bytes(31)
    );
    assertEq(provider.validateCredential(approvedLender, abi.encode(tokenId)), 0, 'short burn auth');
    vm.clearMockedCalls();
  }
