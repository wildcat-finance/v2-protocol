...
import "src/access/PeriodicTermHooks.sol";

contract MockPeriodicTermHooks is PeriodicTermHooks {
    constructor(address _deployer) PeriodicTermHooks(_deployer, "") {}

    function tryValidateAccess(address accountAddress, bytes calldata hooksData)
        external
        returns (bool hasValidCredential, bool wasUpdated)
    {
        ...
        if (wasUpdated) {
            ...
            if (hasValidCredential) {
                emit AccountAccessGranted(status.lastProvider, accountAddress, status.lastApprovalTimestamp);
            } else {
