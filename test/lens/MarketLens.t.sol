...
contract MarketDataTest is BaseMarketTest {
    ...
    function test_coreDelegation_forTokenAndMarketReads() external {
        ...
        vm.expectCall(address(lensCore), abi.encodeWithSelector(IMarketLensCore.getTokenInfo.selector, address(asset)));
        lens.getTokenInfo(address(asset));

        vm.expectCall(address(lensCore), abi.encodeWithSelector(IMarketLensCore.getTokensInfo.selector, singleToken));
        lens.getTokensInfo(singleToken);

        vm.expectCall(
            address(lensCore), abi.encodeWithSelector(IMarketLensCore.getMarketData.selector, address(market))
        );
        lens.getMarketData(address(market));

        vm.expectCall(address(lensCore), abi.encodeWithSelector(IMarketLensCore.getMarketsData.selector, singleMarket));
        lens.getMarketsData(singleMarket);

        vm.expectCall(
            address(lensCore), abi.encodeWithSelector(IMarketLensCore.getMarketDataV2.selector, address(market))
        );
    ...
    function test_coreDelegation_forAccountAndWithdrawalReads() external {
        ...
        LenderAccountQuery memory query =
            LenderAccountQuery({lender: alice, market: address(market), withdrawalBatchExpiries: expiries});
        LenderAccountQuery[] memory queries = new LenderAccountQuery[](1);
        queries[0] = query;

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(IMarketLensCore.getMarketDataWithLenderStatus.selector, alice, address(market))
        );
        lens.getMarketDataWithLenderStatus(alice, address(market));

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(IMarketLensCore.getMarketsDataWithLenderStatus.selector, alice, singleMarket)
        );
        lens.getMarketsDataWithLenderStatus(alice, singleMarket);

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(bytes4(keccak256("getLenderAccountData(address,address)")), alice, address(market))
        );
        lens.getLenderAccountData(alice, address(market));

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(bytes4(keccak256("getLenderAccountData(address,address[])")), alice, singleMarket)
        );
        lens.getLenderAccountData(alice, singleMarket);

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(IMarketLensCore.getLenderAccountsData.selector, address(market), lenders)
        );
        lens.getLenderAccountsData(address(market), lenders);

        vm.expectCall(address(lensCore), abi.encodeWithSelector(IMarketLensCore.queryLenderAccount.selector, query));
        lens.queryLenderAccount(query);

        vm.expectCall(address(lensCore), abi.encodeWithSelector(IMarketLensCore.queryLenderAccounts.selector, queries));
        lens.queryLenderAccounts(queries);

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(IMarketLensCore.getWithdrawalBatchData.selector, address(market), expiries[0])
        );
        lens.getWithdrawalBatchData(address(market), expiries[0]);

        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(IMarketLensCore.getWithdrawalBatchesData.selector, address(market), expiries)
        );
        ...
        vm.expectCall(
            ...
            abi.encodeWithSelector(
                IMarketLensCore.getWithdrawalBatchesDataWithLenderStatus.selector, address(market), expiries, alice
            )
        );
        ...
        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(
                IMarketLensCore.getWithdrawalBatchDataWithLenderStatus.selector, address(market), expiries[0], alice
            )
        );
        ...
        vm.expectCall(
            address(lensCore),
            abi.encodeWithSelector(
                IMarketLensCore.getWithdrawalBatchDataWithLendersStatus.selector, address(market), expiries[0], lenders
            )
        );
        lens.getWithdrawalBatchDataWithLendersStatus(address(market), expiries[0], lenders);
    }

    function checkToken(TokenMetadata memory data, IERC20 token, string memory message) internal view {
    ...
    function checkHooksConfigFlags(HooksConfigData memory actual, HooksConfig expected, string memory labelPrefix)
        ...
        pure
    {
        assertEq(expected.useOnDeposit(), actual.useOnDeposit, string.concat(labelPrefix, "useOnDeposit"));
        assertEq(
        ...
        assertEq(expected.useOnTransfer(), actual.useOnTransfer, string.concat(labelPrefix, "useOnTransfer"));
        ...
        assertEq(expected.useOnCloseMarket(), actual.useOnCloseMarket, string.concat(labelPrefix, "useOnCloseMarket"));
        assertEq(
            expected.useOnNukeFromOrbit(), actual.useOnNukeFromOrbit, string.concat(labelPrefix, "useOnNukeFromOrbit")
        );
        assertEq(
    ...
    function checkRoleProviderData(RoleProviderData memory data, RoleProvider provider, string memory labelPrefix)
        internal
        pure
    {
        assertEq(data.timeToLive, provider.timeToLive(), string.concat(labelPrefix, " timeToLive"));
        assertEq(data.providerAddress, provider.providerAddress(), string.concat(labelPrefix, " providerAddress"));
        assertEq(data.pullProviderIndex, provider.pullProviderIndex(), string.concat(labelPrefix, " pullProviderIndex"));
    }
    ...
    function checkHooksInstance(HooksInstanceData memory data, MarketConfigFuzzInputs memory inputs) internal {
        assertEq(data.hooksAddress, address(hooks), "hooksAddress");
        assertEq(data.borrower, borrower, "borrower");
        assertEq(
            uint256(data.kind),
            inputs.isOpenTermHooks ? uint256(HooksInstanceKind.OpenTerm) : uint256(HooksInstanceKind.FixedTermLoan),
            "kind"
        );
        assertEq(data.hooksTemplate.hooksTemplate, parameters.hooksTemplate, "hooksTemplate");
        assertEq(
            data.hooksTemplate.name, inputs.isOpenTermHooks ? "OpenTermHooks" : "FixedTermHooks", "hooksTemplateName"
        );
        checkConstraints(data.constraints);
        HooksDeploymentConfig deploymentConfig = hooks.config();
        checkHooksConfigFlags(data.deploymentFlags.optional, deploymentConfig.optionalFlags(), "optional ");
        checkHooksConfigFlags(data.deploymentFlags.required, deploymentConfig.requiredFlags(), "required ");
    ...
    function deployPeriodicTermMarket() internal returns (PeriodicTermLensFixture memory fixture) {
        ...
        hooksFactory.addHooksTemplate(fixture.template, "PeriodicTermHooks", address(0), address(0), 0, 0);
        ...
        HooksConfig hooksConfig = deploymentConfig.optionalFlags().setHooksAddress(address(fixture.hooks)).mergeAllFlags(
        ...
        fixture.market =
            WildcatMarket(hooksFactory.deployMarket(deployInputs, hooksData, _nextSalt(borrower), address(0), 0));
        stopPrank();
    }
    ...
    function test_getMarketData(MarketConfigFuzzInputs memory inputs, uint8 conditions) external {
        ...
        assertEq(data.borrower, borrower, "borrower");
        assertEq(data.withdrawalBatchDuration, parameters.withdrawalBatchDuration, "withdrawalBatchDuration");
        ...
        assertEq(data.delinquencyGracePeriod, parameters.delinquencyGracePeriod, "delinquencyGracePeriod");
        ...
        assertEq(
            data.normalizedUnclaimedWithdrawals, state.normalizedUnclaimedWithdrawals, "normalizedUnclaimedWithdrawals"
        );
        assertEq(data.scaledPendingWithdrawals, state.scaledPendingWithdrawals, "scaledPendingWithdrawals");
        assertEq(data.pendingWithdrawalExpiry, state.pendingWithdrawalExpiry, "pendingWithdrawalExpiry");
        ...
        assertEq(data.lastInterestAccruedTimestamp, state.lastInterestAccruedTimestamp, "lastInterestAccruedTimestamp");
        assertEq(
    ...
    function test_getMarketData_supportsPeriodicTermHooks() external {
        ...
        assertEq(uint256(data.hooksConfig.kind), uint256(HooksInstanceKind.PeriodicTerm), "hooksConfig kind");
    ...
    function test_getMarketsData() external view {
        ...
        assertEq(keccak256(abi.encode(data[0])), keccak256(abi.encode(lens.getMarketData(address(market)))), "markets");
    }
    ...
    function checkWithdrawalBatchData(WithdrawalBatchData memory data, uint32 expiry) internal view {
        ...
        assertEq(
            ...
            uint256(
                expiry > block.timestamp
                    ? BatchStatus.Pending
                    : batch.scaledTotalAmount == batch.scaledAmountBurned ? BatchStatus.Complete : BatchStatus.Unpaid
            ),
            "status"
        );
        ...
        uint256 remainder = MathUtils.rayMul(batch.scaledTotalAmount - batch.scaledAmountBurned, market.scaleFactor());

        assertEq(data.normalizedTotalAmount, data.normalizedAmountPaid + remainder, "normalizedTotalAmount");
    }
    ...
    function checkWithdrawalBatchLenderStatus(WithdrawalBatchLenderStatus memory data, uint32 expiry, address lender)
        internal
        view
    {
        ...
        assertEq(data.normalizedAmountWithdrawn, status.normalizedAmountWithdrawn, "normalizedAmountWithdrawn");
        WithdrawalBatch memory batch = market.getWithdrawalBatch(expiry);
    ...
    function checkWithdrawalBatchDataWithLenderStatus(WithdrawalBatchDataWithLenderStatus memory data, uint32 expiry)
        internal
    {
    ...
    function test_getLenderWithdrawalStatus() external {
        ...
        checkWithdrawalBatchDataWithLenderStatus(
            lens.getWithdrawalBatchDataWithLenderStatus(address(market), uint32(expiry), address(alice)), expiry
        );
        fastForward(parameters.withdrawalBatchDuration + 1);
    ...
    function test_getHooksTemplateForBorrower() external {
        ...
        HooksTemplateData memory data = lens.getHooksTemplateForBorrower(address(this), address(hooksTemplate));
    ...
    function test_getHooksTemplateForBorrower_withUnknownTemplate() external {
        ...
        hooksFactory.addHooksTemplate(
            mockTemplate, "MockHooks", address(this), address(originationFeeAsset), 1e18, 1_000
        );

        HooksTemplateData memory data = lens.getHooksTemplateForBorrower(address(this), address(mockTemplate));
    ...
    function checkHooksTemplateData(HooksTemplateData memory data, bool isAccessControl) internal view {
        (address template, string memory name, uint256 index) =
            isAccessControl ? (hooksTemplate, "OpenTermHooks", 0) : (fixedTermHooksTemplate, "FixedTermHooks", 1);
        checkHooksTemplateData(data, template, name, index);
    }

    function checkHooksTemplateData(HooksTemplateData memory data, address template, string memory name, uint256 index)
        internal
        view
    {
        ...
        assertEq(data.totalMarkets, hooksFactory.getMarketsForHooksTemplateCount(data.hooksTemplate), "totalMarkets");
    ...
    function test_getHooksTemplatesForBorrower() external {
        ...
        hooksFactory.updateHooksTemplateFees(
            hooksTemplate, parameters.feeRecipient, address(originationFeeAsset), 1e18, 1_000
        );
        hooksFactory.updateHooksTemplateFees(
            fixedTermHooksTemplate, parameters.feeRecipient, address(originationFeeAsset), 1e18, 1_000
        );

        HooksTemplateData[] memory data = lens.getHooksTemplatesForBorrower(address(this), hooksTemplates);
        assertEq(data.length, 2, "length");
    ...
    function checkLenderStatus(LenderAccountData memory data) internal view {
        ...
        assertEq(data.underlyingApproval, asset.allowance(data.lender, address(market)), "underlyingApproval");
        assertEq(data.isBlockedFromDeposits, status.isBlockedFromDeposits, "isBlockedFromDeposits");
        checkRoleProviderData(data.lastProvider, hooks.getRoleProvider(status.lastProvider), "lastProvider");
        ...
        assertEq(data.isKnownLender, hooks.isKnownLenderOnMarket(data.lender, address(market)), "isKnownLender");
    }
