...
import { DrawnMath } from 'src/libraries/DrawnMath.sol';
import { CROSS_MARKET_GATE_LIB, CLEAN_DOWN_LIB } from 'src/access/covenants/lib/CovenantLibraries.sol';

/// @dev Pins `DrawnMath` to `WildcatMarketRevolving`'s actual
...
contract CovenantBaseTest is MarketConfigMatrix {
  ...
  CombinedCovenantHooks internal hooksInstance;
  ...
  function setUp() public override {
    ...
    covenantTemplate = LibStoredInitCode.deployInitCode(type(CombinedCovenantHooks).creationCode);
    _register();
    _deploy();
  }
  ...
  function _deploy() internal {
    ...
    hooksInstance = CombinedCovenantHooks(instance);
  ...
  function _borrowAndAssert(uint256 amount) internal {
    ...
    uint256 predicted = DrawnMath.drawnAfterBorrow(
      _drawn(),
      market.totalAssets(),
      state,
      amount
  ...
  function _repayAndAssert(uint256 amount) internal {
    MarketState memory state = market.currentState();
    uint256 predicted = DrawnMath.drawnAfterRepay(
      _drawn(),
