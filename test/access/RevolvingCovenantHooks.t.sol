...
/// @dev Integration tests for `RevolvingCovenantHooks`: the clean-down
...
contract RevolvingCovenantHooksTest is MarketConfigMatrix {
  ...
  struct CovenantMarket {
    ...
    RevolvingCovenantHooks hooksInstance;
    uint256 deployedAt;
  }
  ...
  function _deployCovenantMarket(
    ...
    deployed.hooksInstance = RevolvingCovenantHooks(instance);
  ...
  function test_constructor_NotRevolvingFactory() external {
    ...
    stopPrank();
    startPrank(borrower);
    // Constructor asserts the factory name; create2 of the instance fails.
    vm.expectRevert();
    hooksFactory.deployHooksInstance(covenantTemplate, '');
    stopPrank();
  }
