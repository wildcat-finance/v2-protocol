...
contract PeriodicTermHooksTest is BaseAccessControlsTest {
    ...
    function _encodeHooksData(uint128 minimumDeposit) internal pure returns (bytes memory) {
        return abi.encode(FirstWithdrawalWindowStart, PeriodDuration, WithdrawalWindowDuration, minimumDeposit);
    }
    ...
    function _assertPendingAprChange(
        ...
        (PendingAprChange memory pendingAprChange, uint32 actualResponseWindowStart, uint32 actualResponseWindowEnd) =
    ...
    function _assertNoPendingAprChange(address market) internal view {
        (PendingAprChange memory pendingAprChange, uint32 responseWindowStart, uint32 responseWindowEnd) =
            hooks.getPendingAprChange(market);
    ...
    function test_constructor_ExistingProviders(bool isPullProvider1, uint32 ttl1, bool isPullProvider2, uint32 ttl2)
        external
    {
        ...
        inputs.existingProviders[0] =
            ExistingProviderInputs({providerAddress: address(mockProvider1), timeToLive: ttl1});
        inputs.existingProviders[1] =
            ExistingProviderInputs({providerAddress: address(mockProvider2), timeToLive: ttl2});
        ...
        hooks = MockPeriodicTermHooks(address(new PeriodicTermHooks(address(this), abi.encode(inputs))));
        baseHooks = MockBaseAccessControls(address(hooks));
        _validateRoleProviders();
        assertEq(hooks.name(), inputs.name, "name");
    }

    function test_constructor_NewProviders(bool isPullProvider1, uint32 ttl1, bool isPullProvider2, uint32 ttl2)
        external
    {
        ...
        inputs.newProviderInputs[0] =
            CreateProviderInputs({providerFactoryCalldata: abi.encode(salt1, isPullProvider1), timeToLive: ttl1});
        inputs.newProviderInputs[1] =
            CreateProviderInputs({providerFactoryCalldata: abi.encode(salt2, isPullProvider2), timeToLive: ttl2});
        _addExpectedProvider(MockRoleProvider(providerFactory.computeProviderAddress(salt1)), ttl1, isPullProvider1);
        _addExpectedProvider(MockRoleProvider(providerFactory.computeProviderAddress(salt2)), ttl2, isPullProvider2);
        hooks = MockPeriodicTermHooks(address(new PeriodicTermHooks(address(this), abi.encode(inputs))));
        baseHooks = MockBaseAccessControls(address(hooks));
        _validateRoleProviders();
        assertEq(hooks.name(), inputs.name, "name");
    ...
    function test_constructor_NewAndExistingProviders(
        ...
        _addExpectedProvider(mockProvider1, ttl1, isPullProvider1);
        _addExpectedProvider(MockRoleProvider(providerFactory.computeProviderAddress(salt)), ttl2, isPullProvider2);
        hooks = MockPeriodicTermHooks(address(new PeriodicTermHooks(address(this), abi.encode(inputs))));
        baseHooks = MockBaseAccessControls(address(hooks));
        _validateRoleProviders();
        assertEq(hooks.name(), inputs.name, "name");
    ...
    function test_onCreateMarket_FirstWithdrawalWindowStartInFuture() external {
        ...
        assertEq(market.firstWithdrawalWindowStart, futureFirstWithdrawalWindowStart, "firstWithdrawalWindowStart");
        assertFalse(hooks.isWithdrawalWindowOpen(Market), "window closed before future first window start");
    }
    ...
    function test_fuzz_onCreateMarket_InitialWithdrawalWindowTooFarInFuture(uint32 extraDelay) external {
        extraDelay = uint32(bound(extraDelay, 1, 365 days));

        DeployMarketInputs memory inputs;
        uint32 firstWithdrawalWindowStart = PeriodStart + hooks.MaximumInitialWithdrawalWindowDelay() + extraDelay;
    ...
    function test_onCreateMarket_InvalidPeriodDuration() external {
        ...
        hooks.onCreateMarket(
            address(this), Market, inputs, abi.encode(FirstWithdrawalWindowStart, uint32(0), WithdrawalWindowDuration)
        );
    ...
    function test_onCreateMarket_InvalidWithdrawalWindow() external {
        ...
        hooks.onCreateMarket(
            address(this), Market, inputs, abi.encode(FirstWithdrawalWindowStart, PeriodDuration, uint32(0))
        );
        ...
        vm.expectRevert(PeriodicTermHooks.WithdrawalWindowDurationOutOfBounds.selector);
        hooks.onCreateMarket(
            address(this), Market, inputs, abi.encode(FirstWithdrawalWindowStart, PeriodDuration, PeriodDuration)
        );

        vm.expectRevert(PeriodicTermHooks.WithdrawalWindowDurationOutOfBounds.selector);
        hooks.onCreateMarket(
            address(this), Market, inputs, abi.encode(FirstWithdrawalWindowStart, PeriodDuration, PeriodDuration + 1)
        );
    }
    ...
    function test_onCreateMarket_setMinimumDeposit() external {
        ...
        emit PeriodicTermHooks.PeriodicTermUpdated(
            Market, FirstWithdrawalWindowStart, PeriodDuration, WithdrawalWindowDuration
        );
        vm.expectEmit(address(hooks));
    ...
    function test_onCreateMarket_MinimumDepositOverflow() external {
        ...
        hooks.onCreateMarket(
            ...
            Market,
            inputs,
            abi.encode(FirstWithdrawalWindowStart, PeriodDuration, WithdrawalWindowDuration, type(uint136).max)
        );
    }
    ...
    function test_fuzz_isWithdrawalWindowOpen_matchesExplicitWindowMath(uint256 periodIndex, uint256 offsetInPeriod)
        external
    {
    ...
    function test_proposeAnnualInterestBips() external {
        ...
        assertEq(responseWindowEnd, FirstWithdrawalWindowStart + WithdrawalWindowDuration, "responseWindowEnd");
    }
    ...
    function test_fuzz_proposeAnnualInterestBips_BlockedAtEveryWindowOffset(
        ...
        proposedAnnualInterestBips = uint16(bound(proposedAnnualInterestBips, 0, currentAnnualInterestBips - 1));
    ...
    function test_proposeAnnualInterestBips_BetweenWindows() external {
        ...
        assertEq(responseWindowStart, FirstWithdrawalWindowStart + PeriodDuration, "responseWindowStart");
        assertEq(
            responseWindowEnd,
    ...
    function test_proposeAnnualInterestBips_AfterMissedWindows() external {
        MockAprMarket market = new MockAprMarket(1_000);
        uint32 proposalTimestamp = FirstWithdrawalWindowStart + (PeriodDuration * 2) + WithdrawalWindowDuration;
        vm.warp(proposalTimestamp);
        _createMarket(address(market), EmptyHooksConfig, _encodeHooksData());
        hooks.proposeAnnualInterestBips(address(market), 900);
        ...
        assertEq(pendingAprChange.annualInterestBips, 900, "annualInterestBips");
        assertEq(pendingAprChange.proposalTimestamp, proposalTimestamp, "proposalTimestamp");
        assertEq(responseWindowStart, FirstWithdrawalWindowStart + (PeriodDuration * 3), "responseWindowStart");
        assertEq(
            responseWindowEnd,
    ...
    function test_setAnnualInterestAndReserveRatioBips_ReductionAfterWindowCloses() external {
        ...
        vm.prank(address(market));
        (uint16 annualInterestBips, uint16 reserveRatioBips) =
            hooks.onSetAnnualInterestAndReserveRatioBips(500, 0, state, "");
    ...
    function test_setAnnualInterestAndReserveRatioBips_ReductionAtResponseWindowStartIsNotReady() external {
        MockAprMarket market = new MockAprMarket(1_000);
        _createMarket(address(market), EmptyHooksConfig, _encodeHooksData());
        hooks.proposeAnnualInterestBips(address(market), 900);
    ...
    function test_fuzz_setAnnualInterestAndReserveRatioBips_ReductionGateSpec(
        ...
        vm.prank(address(market));
        vm.expectRevert(PeriodicTermHooks.AprChangeNotReady.selector);
        hooks.onSetAnnualInterestAndReserveRatioBips(proposedAnnualInterestBips, requestedReserveRatioBips, state, "");
        ...
        vm.prank(address(market));
        (uint16 annualInterestBips, uint16 reserveRatioBips) = hooks.onSetAnnualInterestAndReserveRatioBips(
            proposedAnnualInterestBips, requestedReserveRatioBips, state, ""
        );
        ...
        (uint16 originalAnnualInterestBips, uint16 originalReserveRatioBips, uint32 temporaryReserveRatioExpiry) =
